# script to produce a 3D graphic of the (old?) view of the hippocampal
# where network is essentially w HPC <-> Neocortex
...
brain_mask = readMNI(what="Brain_Mask", res="2mm");
cut <- 4500;
dtemp <- dim(template);
...
mask_brain = remake_img(vec = img %in% brain_mask, img = img);
mask_hpc = remake_img(vec = img %in% hpc, img = img);
mask_neocortex = remake_img(vec = img %in% neocortex, img = img);
#contour3d(mask_brain, x=1:dtemp[1], y=1:dtemp[2], z=1:dtemp[3], level = 1, alpha = 0.2, draw = TRUE)
...
contour3d(mask_neocortex, level = c(0.5), alpha = c(0.2), add = TRUE, color=c("blue") )
