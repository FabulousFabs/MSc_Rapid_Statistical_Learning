...
for k = 1:size(subjects, 2)
    ...
    subject_ROI_jobs{end+1} = qsubfeval(@qsub_run_subject_ROI, subject, 'memreq', 16*(1024^3), 'timreq', 15*60*1);
end

cd /project/3018012.23/git/analyses/meg/;

save(fullfile(jobdir, jobf), 'subject_ROI_jobs');

%% read qsubs
load(fullfile(jobdir, jobf), 'subject_ROI_jobs');
cd /project/3018012.23/.jobs/;

outputs = {};
for k = 1:size(subject_ROI_jobs, 2)
    outputs{end+1} = qsubget(subject_ROI_jobs{k});
end
